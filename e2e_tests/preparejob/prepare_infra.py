...
def prepare_vault():
    ...
    token, cacrt = get_token_and_cacrt_for_service_account(
        service_account_token_name=operator_token_name, namespace=namespace
    )
    ...
    try:
        k8s_client.CoreV1Api().read_namespaced_config_map(
            name="k8s-itlabs-operator-config", namespace=namespace
        )
    except Exception:
        pass
    else:
...
def debug_vault():
    ...
    client.auth.approle.create_or_update_approle(
        role_name="drole", token_policies=["tester"]
    )
