...
class RabbitClient(AbstractRabbitClient):
    ...
    def create_rabbit_user(self, user: str, password: str):
        ...
        return self._send_rabbit_request(endpoint=f'/users/{user}', method="PUT", data=data)

    def delete_rabbit_user(self, user: str):
        app_logger.info(f"Deleting user '{user}' in rabbit '{self.url}'")
        self._send_rabbit_request(endpoint=f"/users/{user}", method="DELETE", data={})
    ...
    def create_rabbit_vhost(self, vhost: str):
        ...
        return self._send_rabbit_request(endpoint=f"/vhosts/{vhost}", method="PUT", data={})

    def delete_rabbit_vhost(self, vhost: str):
        app_logger.info(f"Creating vhost '{vhost}' in rabbit {self.url}")
        self._send_rabbit_request(endpoint=f"/vhosts/{vhost}", method="DELETE", data={})

    def get_user_vhost_permissions(self, user, vhost):
        return self._send_rabbit_request(endpoint=f'/permissions/{vhost}/{user}')

    def create_user_vhost_permissions(self, user, vhost):
        app_logger.info(f"Configuring user '{user}' privileges to vhost '{vhost}' in rabbit '{self.url}'")
        data = {"configure": ".*", "write": ".*", "read": ".*"}
        return self._send_rabbit_request(endpoint=f'/permissions/{vhost}/{user}', method='PUT', data=data)

    def _send_rabbit_request(self, endpoint, data=None, method='GET'):
        endpoint = join(self.url, f'/api{endpoint}')

        encoded_user_pass = base64.b64encode(f'{self.user}:{self.password}'.encode('UTF-8')).decode('UTF-8')
        headers = {
