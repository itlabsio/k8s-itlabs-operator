...
from clients.sentry.dto_factories import (
    SentryProjectDtoFactory,
    SentryProjectKeyDtoFactory,
    SentryTeamDtoFactory,
)
...
class SentryClient(AbstractSentryClient):
    ...
    def _send_request(
        self, endpoint: str, data: Optional[dict] = None, method: str = "GET"
    ):
        ...
        try:
            ...
            raise InfrastructureServiceProblem(
                "Sentry", SentryClientError(response)
            )
    ...
    def get_sentry_team(self, team_slug: str) -> Optional[SentryTeam]:
        response = self._send_request(
            endpoint=f"/teams/{self.organization}/{team_slug}/"
        )
        if response:
    ...
    def create_sentry_team(
        ...
        data = SentryTeamDtoFactory.dict_from_dto(
            SentryTeam(name=team_name, slug=team_slug)
        )
        response = self._send_request(
            endpoint=f"/organizations/{self.organization}/teams/",
            method="POST",
            data=data,
        )
        return SentryTeamDtoFactory.dto_from_dict(response)
    ...
    def delete_sentry_team(self, team_name: str):
        self._send_request(
            endpoint=f"/teams/{self.organization}/{team_name}/", method="DELETE"
        )
    ...
    def create_sentry_project(
        ...
        data = SentryProjectDtoFactory.dict_from_dto(
            SentryProject(name=project_name, slug=project_slug)
        )
        response = self._send_request(
            endpoint=f"/teams/{self.organization}/{team_slug}/projects/",
            method="POST",
            data=data,
        )
        return SentryProjectDtoFactory.dto_from_dict(response)
    ...
    def get_sentry_project_keys(
        ...
        response = self._send_request(
            endpoint=f"/projects/{self.organization}/{project_slug}/keys/"
        )
        return (
            [SentryProjectKeyDtoFactory.dto_from_dict(r) for r in response]
            if response
            else []
        )
