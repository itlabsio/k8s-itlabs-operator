...
class VaultService(AbstractVaultService):
    ...
    def get_sentry_ms_credentials(
        self, vault_path: str
    ) -> Optional[SentryMsSecretDto]:
    ...
    def unvault_sentry_connector(
        ...
        if not (
            sentry_connector.url
            and sentry_connector.token
            and sentry_connector.organization
        ):
            return None
        return SentryApiSecretDtoFactory.api_secret_dto_from_connector(
            sentry_connector
        )
