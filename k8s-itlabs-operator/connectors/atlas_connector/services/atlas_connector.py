...
from connectors.atlas_connector.dto import (
    AtlasConfigDto,
    AtlasConnectorAnnotations,
    AtlasMicroserviceDto,
)
from connectors.atlas_connector.factories.dto_factory import (
    AtlasMicroserviceDtoFactory,
)
from connectors.atlas_connector.factories.service_factories.atlas import (
    AtlasServiceFactory,
)
from connectors.atlas_connector.factories.service_factories.vault import (
    VaultServiceFactory,
)
...
class AtlasConnectorService:
    ...
    @classmethod
    def on_upsert_pod(
        cls, namespace: str, annotations: AtlasConnectorAnnotations
    ) -> ConnectorStatus:
        status = ConnectorStatus()
        status.is_used = annotations.is_connector_enabled
        if not status.is_used:
            logging.info(
                "Atlas connector is not used, because no expected annotations"
            )
            return status
        ...
        try:
            ...
            logging.error(
                "Problem with infrastructure, some changes may not be applied",
                exc_info=e,
            )
            status.exception = e
        return status
    ...
    def update_microservice(
        cls,
        atlas_config_dto: AtlasConfigDto,
        atlas_ms_dto: AtlasMicroserviceDto,
    ):
        """send to atlas info about service and environment"""
        vault_service = VaultServiceFactory.create_vault_service()
        atlas_token = vault_service.get_atlas_token(
            vault_path=atlas_config_dto.vault_path
        )
        atlas_service = AtlasServiceFactory.create_atlas_service(
            atlas_url=atlas_config_dto.atlas_url, atlas_token=atlas_token
        )
