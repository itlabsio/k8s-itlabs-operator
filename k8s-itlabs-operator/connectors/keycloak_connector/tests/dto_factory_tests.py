...
class TestKeycloakConnectorMicroserviceDtoFactory:
    ...
    def test_required_annotations_are_empty(self):
        ...
        with pytest.raises(KeycloakConnectorAnnotationEmptyValueError) as e:
            KeycloakConnectorMicroserviceDtoFactory.dto_from_metadata(annotations)
        assert len(e.value.empty_annotation_names) == len(specifications.KEYCLOAK_CONNECTOR_REQUIRED_ANNOTATIONS)
        assert specifications.KEYCLOAK_CLIENT_ID_ANNOTATION in e.value.empty_annotation_names
        assert specifications.KEYCLOAK_VAULT_PATH_ANNOTATION in e.value.empty_annotation_names
        assert specifications.KEYCLOAK_INSTANCE_NAME_ANNOTATION in e.value.empty_annotation_names
