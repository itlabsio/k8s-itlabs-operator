...
from connectors.keycloak_connector.exceptions import (
    ...
    KeycloakConnectorInfrastructureError,
)
...
class KeycloakConnectorValidationService(ConnectorValidationService):
    ...
    def validate(
        self, keycloak_connector_dto: KeycloakConnectorMicroserviceDto
    ) -> List[ConnectorError] | None:
        self.errors = []
    ...
    def _check_instance(self, instance_name: str):
        instance_connector = self._kube_service.get_keycloak_connector(
            instance_name
        )
