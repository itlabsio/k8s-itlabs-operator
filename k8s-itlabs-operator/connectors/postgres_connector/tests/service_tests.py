...
from connectors.postgres_connector.tests.factories import (
    PgConnectorInstanceSecretDtoTestFactory,
    PgConnectorMicroserviceDtoTestFactory,
)
from connectors.postgres_connector.tests.mocks import (
    KubernetesServiceMocker,
    MockedPostgresService,
    MockedVaultService,
    MockKubernetesService,
    PostgresServiceFactoryMocker,
)
...
class TestPostgresConnectorService:
    def test_get_or_create_db_credentials_when_cred_was_in_vault(self):
        ...
        ms_pg_cred = PgConnectorDbSecretDtoFactory.dto_from_ms_pg_con(
            pg_instance_cred=pg_instance_cred, ms_pg_con=ms_pg_con
        )
        pg_con_service = PostgresConnectorService(
            vault_service=MockedVaultService(
                pg_instance_cred=pg_instance_cred, ms_pg_cred=ms_pg_cred
            )
        )
        db_creds = pg_con_service.get_or_create_db_credentials(
            pg_instance_cred=pg_instance_cred, ms_pg_con=ms_pg_con
        )
        ...
        assert (
            pg_con_service.vault_service.create_pg_ms_credentials_call_count
            == 0
        )
    ...
    def test_get_or_create_db_credentials_when_cred_was_not_in_vault(self):
        ...
        pg_con_service = PostgresConnectorService(
            vault_service=MockedVaultService(pg_instance_cred=pg_instance_cred)
        )
        db_creds = pg_con_service.get_or_create_db_credentials(
            pg_instance_cred=pg_instance_cred, ms_pg_con=ms_pg_con
        )
    ...
    def test_on_create_deployment(self, mocker):
        ...
        kube_mocker = KubernetesServiceMocker.mock_get_pg_connector(
            mocker, pg_connector
        )
        mocked_pg_service = MockedPostgresService()
        PostgresServiceFactoryMocker.mock_create_pg_service(
            mocker, mocked_pg_service
        )
            ...
            "containers": [
                {
                    "name": "first",
                    "env": [
                        {"name": var_name[0], "value": "some_value"}
                        for var_name in specifications.DATABASE_VAR_NAMES
                    ],
                }
            ]
        }
        assert not pg_con_service.mutate_containers(
            spec=spec, ms_pg_con=ms_pg_con
        )
    ...
    def test_mutate_containers_variables_not_in_container(self):
        ...
        assert (
            pg_con_service.vault_service.get_vault_env_value_call_count
            == len(specifications.DATABASE_VAR_NAMES)
        )

    def test_mutate_containers_variables_already_in_onr_container_not_in_enother(
        self,
    ):
        ms_pg_con: PgConnectorMicroserviceDto = (
            PgConnectorMicroserviceDtoTestFactory()
...
class TestVaultService:
    def test_get_vault_env_value(self):
        ...
        vault_key = "vault_key"
        vault_env_value = vault_service.get_vault_env_value(
...
class TestPostgresConnectorValidationService:
    ...
    def test_annotation_contain_incorrect_vault_secret(
        self, vault, kube, postgres
    ):
