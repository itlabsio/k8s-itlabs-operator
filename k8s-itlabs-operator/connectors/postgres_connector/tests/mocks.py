...
class MockedVaultService(AbstractVaultService):
    def __init__(
        self,
        pg_instance_cred: Optional[PgConnectorInstanceSecretDto] = None,
...
class MockedPostgresService(AbstractPostgresService):
    ...
    def grant_access_on_select(
        self,
        grantor_name: str,
        grantor_password: str,
        grantor_database: str,
        grantee_name: str,
    ):
        pass
...
class KubernetesServiceMocker:
    @staticmethod
    def mock_get_pg_connector(
        mocker, pg_connector: Optional[PgConnector] = None
    ):
        return mocker.patch(
...
class PostgresServiceFactoryMocker:
    @staticmethod
    def mock_create_pg_service(
        mocker, pg_service: Optional[AbstractPostgresService]
    ):
        return mocker.patch(
...
class MockKubernetesService(AbstractKubernetesService):
    def __init__(
        ...
        readonly_username: (
            str | None
        ) = "vault:secret/data/infrastructure/postgres#READONLY_USERNAME",
    ):
