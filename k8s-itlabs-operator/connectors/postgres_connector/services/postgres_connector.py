...
from connectors.postgres_connector.exceptions import (
    ...
    PgConnectorReadonlyUsernameDoesNotExist,
    PgConnectorReadonlyUsernameIsNotSet,
...
class PostgresConnectorService:
    ...
    def on_create_deployment(self, ms_pg_con: PgConnectorMicroserviceDto):
        pg_connector = KubernetesService.get_pg_connector(
            ms_pg_con.pg_instance_name
        )
        if not pg_connector:
        ...
        with ConnectorSourceLock(source_hash):
            db_creds = self.get_or_create_db_credentials(
                pg_instance_cred, ms_pg_con
            )
            ...
            if ms_pg_con.grant_access_for_readonly_user:
                ...
                if not pg_service.is_user_exist(
    ...
    def get_or_create_db_credentials(
        ...
        pg_ms_creds = self.vault_service.get_pg_ms_credentials(
            ms_pg_con.vault_path
        )
        if pg_ms_creds:
            ...
            pg_ms_creds = PgConnectorDbSecretDtoFactory.dto_from_ms_pg_con(
                pg_instance_cred, ms_pg_con
            )
            self.vault_service.create_pg_ms_credentials(
                ms_pg_con.vault_path, pg_ms_creds
            )
        return pg_ms_creds

    def mutate_containers(
        self, spec: dict, ms_pg_con: PgConnectorMicroserviceDto
    ) -> bool:
        mutated = False
        for container in spec.get("containers", []):
            mutated = self.mutate_container(
                container, mutated, ms_pg_con.vault_path
            )
        for init_container in spec.get("initContainers", []):
            mutated = self.mutate_container(
                init_container, mutated, ms_pg_con.vault_path
            )
        return mutated
