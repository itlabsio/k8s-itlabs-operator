...
from connectors.rabbit_connector.tests.factories import (
    RabbitApiSecretDtoTestFactory,
    RabbitConnectorMicroserviceDtoTestFactory,
)
from connectors.rabbit_connector.tests.mocks import (
...
class TestRabbitConnectorService:
    def test_on_create_deployment_no_crds(self, mocker):
        ...
        with pytest.raises(RabbitConnectorCrdDoesNotExist):
            rabbit_connector_service.on_create_deployment(
                ms_rabbit_con=ms_rabbit_con
            )
    ...
    def test_on_create_deployment_no_rabbit_in_crds(self, mocker):
        ...
        KubernetesServiceMocker.mock_get_rabbit_connector(
            mocker=mocker, rabbit_connector=rabbit_connector
        )
        ms_rabbit_con = RabbitConnectorMicroserviceDtoTestFactory()
        rabbit_connector_service = RabbitConnectorService(
            vault_service=MockedVaultService()
    ...
    def test_mutate_containers_variables_already_in_container(self):
        ...
        assert not rabbit_con_service.mutate_containers(
            spec=spec, ms_rabbit_con=ms_rabbit_con
        )
    ...
    def test_mutate_containers_variables_not_in_container(self):
        ...
        assert rabbit_con_service.mutate_containers(
            spec=spec, ms_rabbit_con=ms_rabbit_con
        )
        assert (
            rabbit_con_service.vault_service.get_vault_env_value_call_count
            == len(specifications.RABBIT_VAR_NAMES)
        )

    def test_mutate_containers_variables_already_in_one_container_not_in_enother(
        self,
    ):
        ms_rabbit_con: RabbitConnectorMicroserviceDto = (
            RabbitConnectorMicroserviceDtoTestFactory()
