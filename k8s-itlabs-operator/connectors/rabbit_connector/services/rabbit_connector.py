...
from connectors.rabbit_connector.exceptions import (
    NotMatchingUsernames,
    NotMatchingVhostNames,
...
from connectors.rabbit_connector.factories.service_factories.rabbit import (
    RabbitServiceFactory,
)
...
class RabbitConnectorService:
    ...
    def on_create_deployment(
        self, ms_rabbit_con: RabbitConnectorMicroserviceDto
    ):
        rabbit_connector = KubernetesService.get_rabbit_connector(
            ms_rabbit_con.rabbit_instance_name
        )
        ...
        rabbit_instance_cred = self.vault_service.unvault_rabbit_connector(
            rabbit_connector
        )
        ...
        rabbit_service = RabbitServiceFactory.create_rabbit_service(
            rabbit_instance_cred
        )
        ...
        with ConnectorSourceLock(source_hash):
            rabbit_ms_creds = self.get_or_create_rabbit_credentials(
                rabbit_instance_cred, ms_rabbit_con
            )
    ...
    def get_or_create_rabbit_credentials(
        self,
        rabbit_api_cred: RabbitApiSecretDto,
        ms_rabbit_con: RabbitConnectorMicroserviceDto,
    ) -> RabbitMsSecretDto:
        rabbit_ms_creds = self.vault_service.get_rabbit_ms_credentials(
        ...
        if rabbit_ms_creds:
            ...
            rabbit_ms_creds = RabbitMsSecretDtoFactory.dto_from_ms_rabbit_con(
                rabbit_api_cred, ms_rabbit_con
            )
            self.vault_service.create_ms_rabbit_credentials(
                ms_rabbit_con.vault_path, rabbit_ms_creds
            )
        return rabbit_ms_creds

    def mutate_containers(
        self, spec, ms_rabbit_con: RabbitConnectorMicroserviceDto
    ):
        mutated = False
        for container in spec.get("containers", []):
            mutated = self.mutate_container(
                container, mutated, ms_rabbit_con.vault_path
            )
        for init_container in spec.get("initContainers", []):
            mutated = self.mutate_container(
                init_container, mutated, ms_rabbit_con.vault_path
            )
        return mutated
