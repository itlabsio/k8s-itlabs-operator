...
class TestMonitoringConnectorMicroserviceDtoFactory:
    def test_dto_empty_annotations(self):
        dto = MonitoringConnectorMicroserviceDtoFactory.dto_from_annotations(
            annotations={}
        )
    ...
    def test_dto_full_annotations(self):
        ...
        assert (
            dto.interval
            == annotations[specifications.MONITORING_INTERVAL_NAME_ANNOTATION]
        )
        assert (
            dto.metric_path
            == annotations[specifications.MONITORING_PATH_NAME_ANNOTATION]
        )
