...
REQUIRED_VAULT_SECRET_KEYS = {
    vault_key for env_name, vault_key in specifications.DATABASE_VAR_NAMES
}
REQUIRED_POD_ENVIRONMENTS = {
    env_name for env_name, vault_key in specifications.DATABASE_VAR_NAMES
}
...
def get_postgres_readonly_username():
    return os.getenv("POSTGRES_READONLY_USERNAME", POSTGRES_READONLY_USERNAME)
...
def create_postgres_cr(
    ...
    pg_secret,
    pg_cr,
    pg_cr_without_readonly,
    pg_cr_with_non_exist_readonly,
):
    secret = vault.read_secret(POSTGRES_VAULT_SECRET_PATH)
    if not secret:
        vault.create_secret(POSTGRES_VAULT_SECRET_PATH, pg_secret)
...
def test_postgres_operator_on_initial_deployment_application(
    k8s, vault, app_name
):
    ...
    assert not any(
        event.reason == "PostgresConnector" and app_name in event.regarding.name
...
def use_non_exist_instance():
    with mock.patch.dict(
        os.environ, {"POSTGRES_INSTANCE_NAME": "non-exist-instance"}
    ):
        yield
...
def test_postgres_operator_on_deployment_using_non_exist_custom_resource(
    k8s, vault, app_name
):
    ...
    assert any(
        ...
        and "Postgres Custom Resource `non-exist-instance` does not exist"
        in event.note
...
@pytest.mark.e2e
@pytest.mark.usefixtures(
    "use_instance_without_readonly_username",
    "deploy_app",
    "wait_app_deployment",
)
def test_test_postgres_operator_on_deployment_using_custom_resource_without_readonly(
    k8s, app_name
):
    # Event was created
    events = EventsV1Api(k8s).list_namespaced_event(
    ...
    assert any(
        ...
        and event.reason == "PostgresConnector"
        and (
            "Username for readonly access to the database is not set in "
            "Custom Resource `postgres-without-ro` for Postgres"
        )
        in event.note
        and app_name in event.regarding.name
        for event in events.items
...
@pytest.mark.e2e
@pytest.mark.usefixtures(
    "use_instance_contain_non_exist_readonly_username",
    "deploy_app",
    "wait_app_deployment",
)
def test_test_postgres_operator_on_deployment_using_custom_resource_contain_non_exist_readonly(
    k8s, app_name
):
    # Event was created
    events = EventsV1Api(k8s).list_namespaced_event(
    ...
    assert any(
        ...
        and event.reason == "PostgresConnector"
        and (
            "Username for readonly access to the database does not exist "
            "in `postgres-with-non-exist-ro`"
        )
        in event.note
        and app_name in event.regarding.name
        for event in events.items
