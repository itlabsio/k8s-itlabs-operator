...
class RootKeycloakClient(KeycloakClient):
    ...
    def get_realm_user_id(self, realm: str, username: str) -> Optional[str]:
        path = self._build_path(URL_ADMIN_USER.format(realm=realm, username=username))
    ...
    def get_realm_client_id(self, realm: str, client: str) -> Optional[str]:
        path = self._build_path(URL_ADMIN_CLIENTS.format(realm=realm, client=client))
        response = requests.get(path, auth=self._get_auth())
        try:
            return response.json()[0]["id"]
        except (IndexError, KeyError):
    ...
    def get_realm_client_role_id(self, realm: str, client_id: str, role: str) -> Optional[str]:
        path = self._build_path(URL_ADMIN_CLIENT_ROLE.format(realm=realm, client_id=client_id, role=role))
        response = requests.get(path, auth=self._get_auth())
        try:
            return response.json()[0]["id"]
        except (IndexError, KeyError):
    ...
    def assign_manage_clients_role_for_realm_user(self, realm: str, user_id: str):
        ...
        path = self._build_path(URL_ADMIN_ASSIGN_USER_ROLE_MAPPING.format(
