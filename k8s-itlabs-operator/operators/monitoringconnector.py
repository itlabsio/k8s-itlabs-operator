...
import kopf
from connectors.monitoring_connector.factories.dto_factory import (
...
def create_services(namespace, name, annotations, **_):
    ...
    monitoring_connector_service = (
        MonitoringConnectorServiceFactory.create_monitoring_connector_service()
    )
    status.is_used = (
        monitoring_connector_service.is_monitoring_connector_used_by_object(
            annotations
        )
    )
    if status.is_used:
        ms_mon_con_dto = (
            MonitoringConnectorMicroserviceDtoFactory.dto_from_annotations(
                annotations
            )
        )
        created = monitoring_connector_service.create_service_monitor(
            ms_mon_con_dto, name, namespace
        )
        status.is_enabled = created
    else:
        logging.info(
            "Monitoring connector is not used, because no expected annotations"
        )
