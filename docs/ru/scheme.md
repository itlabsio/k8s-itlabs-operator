...
оператором для создания Keycloak клиента. Данные настройки хранятся на стороне кластера. Манифест
...
(см. _"Схема работы": шаг 3_). Данные настройки хранятся на стороне кластера. Манифест _PostgresConnector_
...
spec:
  ...
  - name: postres-stage
  # Путь к секрету в Vault, где находятся настройки подключения к Postgres.
  # Секрет должен содержать следующие ключи:
  #   - DATABASE_HOST - хост по которому доступен Postgres (postgres-stage);
  #   - DATABASE_PORT - порт по которому доступен Postgres (5432);
  #   - DATABASE_USER - логин привилегированной учетной записи;
  #   - DATABASE_PASSWORD - пароль привилегированной учетной записи;
  #   - DATABASE_NAME - имя базы данных (postgres);
  #   - DATABASE_KUBE_DOMAIN - внутрикластерный адрес Postgres.
    vaultpath: "vault:secret/data/postgres-stage"
  - name: postres-production
    vaultpath: "vault:secret/data/postgres-production"
...
(см. _"Схема работы": шаг 3_). Настройки хранятся на стороне кластера. Манифест _RabbitConnector_
выглядит следующим образом:
...
spec:
  ...
  - name: rabbit-stage
  # Путь к секрету в Vault, где находятся настройки подключения к Rabbit.
  # Секрет должен содержать следующие ключи:
  #   - API_URL - полный путь до Rabbit (http://rabbit-stage:15672);
  #   - API_USER - логин привилегированной учетной записи;
  #   - API_PASSWORD - пароль привилегированной учетной записи;
  #   - BROKER_HOST -  хост по которому доступен Rabbit (rabbit-stage);
  #   - BROKER_PORT - порт по которому доступен Rabbit (15672).
    vaultpath: "vault:secret/data/rabbit-stage"
  - name: rabbit-production
    vaultpath: "vault:secret/data/rabbit-production"
...
приложения (см. _"Схема работы": шаг 3_). Данные настройки хранятся на стороне кластера.
...
metadata:
  ...
  name: sentry.io
spec:
  # Путь к секрету в Vault, где находятся настройки подключения к Sentry.
  # Секрет должен содержать следующие ключи:
  #   - API_TOKEN - токен доступа к API Sentry (см.: https://docs.sentry.io/api/auth/);
  #   - API_URL - адрес по которому доступен Sentry (sentry.io).
  vaultpath: "vault:secret/data/sentry"
